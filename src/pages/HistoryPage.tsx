...
const HistoryPage = () => {
  const { files, categories } = useFiles();
  ...
  const filtered = useMemo(() => {
    ...
    if (categoryFilter !== "all") {
      result = result.filter((f) => f.category_name === categoryFilter);
    }
    result.sort((a, b) => {
      let cmp = 0;
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-secondary/30">
                  <th className="px-4 py-3 text-left font-display font-semibold text-muted-foreground cursor-pointer hover:text-foreground" onClick={() => toggleSort("name")}>
                    <span className="flex items-center gap-1">File Name <ArrowUpDown size={12} /></span>
                  </th>
                  <th className="px-4 py-3 text-left font-display font-semibold text-muted-foreground">Category</th>
                  <th className="px-4 py-3 text-left font-display font-semibold text-muted-foreground hidden sm:table-cell">Confidence</th>
                  <th className="px-4 py-3 text-left font-display font-semibold text-muted-foreground cursor-pointer hover:text-foreground" onClick={() => toggleSort("date")}>
                    <span className="flex items-center gap-1">Date <ArrowUpDown size={12} /></span>
                  </th>
                  <th className="px-4 py-3 text-left font-display font-semibold text-muted-foreground cursor-pointer hover:text-foreground hidden sm:table-cell" onClick={() => toggleSort("size")}>
                    <span className="flex items-center gap-1">Size <ArrowUpDown size={12} /></span>
                  </th>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span className="inline-flex items-center gap-1 rounded-full bg-primary/10 border border-primary/20 px-2 py-0.5 text-xs text-primary">
                        {getIcon(file.category_name)} {file.category_name}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-muted-foreground hidden sm:table-cell">
                      {file.status === "done" ? `${file.confidence}%` : file.status}
                    </td>
                    <td className="px-4 py-3 text-muted-foreground">
                      {new Date(file.created_at).toLocaleDateString()}
                    </td>
                    <td className="px-4 py-3 text-muted-foreground hidden sm:table-cell">{formatFileSize(file.size)}</td>
                    <td className="px-4 py-3">
                      <button
